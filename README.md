...
- 管理员 AI 智能创建窗口：文字/文本文件识别、能力拆分、四维评分、风险提示、补充要求再生成和确认安装
...
管理员在“公司能力库”打开一个轻量配置窗口，可以粘贴文字或上传 TXT、Markdown、JSON、YAML、TOML、XML、CSV 文件（最大 100 KB）。公司上游 AI 会把素材识别为一个或多个 Agent、Skill、Prompt 候选项，并给出综合、清晰度、复用性、安全性评分，结论和风险提示。管理员可以补充要求后重新分析，勾选候选项并确认安装。

- 这是能力生产工具，不是通用对话入口。
- AI 评分只提供决策辅助；所有候选默认不勾选，管理员检查完整指令后必须逐项主动选择，`REJECT` 候选不能安装。
- 多项安装在数据库事务中完成；新 Agent 可绑定同批次新建或已存在的公司 Skill。
- 文件只在浏览器读取为文字，应用不保存原始文件；关闭窗口或退出登录会清空浏览器内的素材和候选项并作废在途分析。分析内容会发送给管理员配置的公司上游 AI。请遵守该供应商的数据处理政策，不要上传未经授权的秘密或个人数据。
- 只安装声明式指令，不执行上传文件或 AI 输出中的代码。
